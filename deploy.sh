...
docker run -d --name with_dashboard -p 9001:9001 webservice\
	-e dashboard=True \
	-e google=GOOGLE_TRANSLATE_API_KEY \
	-e microsoft=MICROSOFT_TRANSLATE_API_KEY
...
docker run -d --name without_dashboard -p 9001:9001 webservice\
	-e dashboard=False \
	-e google=GOOGLE_TRANSLATE_API_KEY \
	-e microsoft=MICROSOFT_TRANSLATE_API_KEY
